...
datos_random = ["Los bebés nacen con aproximadamente 300 huesos, que se fusionan y reducen a 206 en la edad adulta","Venus es el único planeta que gira en sentido de las agujas del reloj.", "Cleopatra vivió más cerca de la invención del iPhone que de la construcción de las pirámides de Giza.", "Las jirafas tienen muchas más probabilidades de ser alcanzadas por un rayo que los humanos. "]

@app.route("/")
def home():
    return """<h1>Esta es la página principal</h1>"
    "<a href="/random_fact">¡Ver un dato aleatorio sobre el ambiente!</a>
    "<a href="/random_datos">¡Ver un dato random de cualquier cosa!</a>"""
...
@app.route("/random")
def random_datos():
    return f'<p>{random.choice(datos_random)}</p>'
